...
public class SolutionOA {
    ...
    /*
     * Subarray Imbalance/Student Ranks for each subarray, after sorting, if A[i] -
     * A[i - 1] > 1
     * then imbalance + 1 for A[i], as long as there is a A[j] = A[i] + 1, then A[i]
     * cannot make
     * contribution x x x 4 2 [3] 1 5 x x x l i r 23 3 3 31 (i - l) * (r - i) there
     * should be at
     ...
     * 
     * x x l [x x x i x x x x] r x x left side: (A[i] * sum of occurence of each
     * element) * (r - i);
     * x x x i, x x i, x i, i sum of all subArrSum: subArrSumSqaure: A[l + 1] + 2 *
     * A[l + 2] + 3 *
     * A[l + 3] + ... + (i - l) * A[k] a b c d e f g h l i subArrSumSquare_left[i]:
     * a + 2b + 3c + 4d
     * + 5e + 6f + 7g + 8h what we need is: c + 2d + 3e + 4d + 5g + 6h
     * subArrSumSqaure[i] -
     * subArrSumSqaure[l] - (prefisSum[i] - prefixSum[l]) * (l + 1) a b c d e f g h
     * i r
     * subArrSumSqaure_right[i]: 8a + 7b + 6c + 5d + 4e + 3f + 2g + h what we need
     * is: 6a + 5b + 4c
     * + 3d + 2e + f subArrSumSqaure_right[i] - subArrSumSqaure_right[r] -
     * (prefixSum[r - 1] -
     * prefixSum[i - 1]) * (m - r)
     */
