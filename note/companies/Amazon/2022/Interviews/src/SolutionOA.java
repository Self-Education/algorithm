...
public class SolutionOA {
    ...
    /*
     * Subarray Imbalance/Student Ranks for each subarray, after sorting, if A[i] - A[i - 1] > 1
     * then imbalance + 1 for A[i], as long as there is a A[j] = A[i] + 1, then A[i] cannot make
     * contribution x x x 4 2 [3] 1 5 x x x l i r 23 3 3 31 (i - l) * (r - i) there should be at
     * least one larger number than A[i] l [x x pl] (x x i) x [nl x x] r x
     */
    public int subarrayImbalance(int[] arr) {
        int m = arr.length;
        // find the index of A[i] + 1
        Map<Integer, Integer> map = new HashMap<>();
        int[] leftBound = new int[m], rightBound = new int[m];
        Arrays.fill(leftBound, -1);
        Arrays.fill(rightBound, m);
        for (int i = 0; i < m; i++) {
            if (map.containsKey(arr[i] + 1)) {
                leftBound[i] = map.get(arr[i] + 1);
            }
            map.put(arr[i], i);
        }
        map = new HashMap<>();
        ...
        stack = new Stack<>();
        int[] nextLarger = new int[m];
        Arrays.fill(nextLarger, m);
        for (int i = 0; i < m; i++) {
            while (!stack.isEmpty() && arr[stack.peek()] < arr[i]) {
        ...
        int ans = 0;
        for (int i = 0; i < m; i++) {
            int left = i - leftBound[i];
            int right = rightBound[i] - i;
            int deduct = (i - prevLarger[i]) * (nextLarger[i] - i);
            ans += left * right - deduct;
        }
        return ans;
     ...
     * 
     * x x l [x x x i x x x x] r x x left side: (A[i] * sum of occurence of each element) * (r - i);
     * x x x i, x x i, x i, i sum of all subArrSum: subArrSumSqaure: A[l + 1] + 2 * A[l + 2] + 3 *
     * A[l + 3] + ... + (i - l) * A[k] a b c d e f g h l i subArrSumSquare_left[i]: a + 2b + 3c + 4d
     * + 5e + 6f + 7g + 8h what we need is: c + 2d + 3e + 4d + 5g + 6h subArrSumSqaure[i] -
     * subArrSumSqaure[l] - (prefisSum[i] - prefixSum[l]) * (l + 1) a b c d e f g h i r
     * subArrSumSqaure_right[i]: 8a + 7b + 6c + 5d + 4e + 3f + 2g + h what we need is: 6a + 5b + 4c
     * + 3d + 2e + f subArrSumSqaure_right[i] - subArrSumSqaure_right[r] - (prefixSum[r - 1] -
     * prefixSum[i - 1]) * (m - r)
     */
    public long countPowerForSerives(int[] arr) {
        int m = arr.length;
        ...
        long[] sumSquareLeft = new long[m];
        sumSquareLeft[0] = arr[0];
        for (int i = 1; i < m; i++)
            sumSquareLeft[i] = sumSquareLeft[i - 1] + arr[i] * (i + 1);

        long[] sumSquareRight = new long[m];
        sumSquareRight[m - 1] = arr[m - 1];
        for (int i = m - 2; i >= 0; i--)
            sumSquareRight[i] = sumSquareRight[i + 1] + arr[i] * (m - i);
        ...
        // System.out.println(Arrays.toString(sumSquareLeft));
        // System.out.println(Arrays.toString(sumSquareRight));
        ...
        for (int i = 0; i < m; i++) {
            int l = prevSmaller[i];
            long sum1 = sumSquareLeft[i];
            if (l > 0) {
                sum1 -= sumSquareLeft[l];
            }
            sum1 -= (prefixSum[i] - (l >= 0 ? prefixSum[l] : 0)) * (l + 1);

            int r = nextSmaller[i];
            long sum2 = sumSquareRight[i];
            if (r < m)
                sum1 -= sumSquareRight[r];
            sum2 -= (prefixSum[r - 1] - (i > 0 ? prefixSum[i - 1] : 0)) * (m - r);

            ans += arr[i] * (sum1 * (r - i) + sum2 * (i - l) - arr[i] * (i - l) * (r - i));
        }
        return ans;
    ...
    public int badNumbers(int[] arr, int low, int high) {
        ...
        if (i == m) {
            ...
            maxGap = Math.max(maxGap, arr[i - 1] - high);
        }
        return maxGap;
    }
     ...
     */
    public List<int[]> OptimizeUtilization(List<int[]> A, List<int[]> B, int target) {
