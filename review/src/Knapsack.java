...
public class Knapsack {
    ...
    /*
     * given two integer arrays val[0..n-1] and wt[0..n-1], find the maximum value
     * sum of subset of
     * val[] such that sum of the weight of this subset no larger than W, 1<=
     * wt[i]<= 1000, 1<= val[i]<=1000
     */
    int ans_01;
    ...
    // find elements in val from index 0 to n(inclusive) so that sum of those
    // elments are max
    ...
    public int naive(int[] val, int[] wt, int weight, int n) {
        ...
        if (n == 0) {
            ...
            // include current element wt[n], in order to make current total weight <=
            // weight
    ...
    public int helper(int[] val, int[] wt, int weight, int n) {
        if (memo[n][weight] != Integer.MIN_VALUE)
